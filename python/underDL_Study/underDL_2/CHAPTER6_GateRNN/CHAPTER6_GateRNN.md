...
RNNLM의 개선 포인트 3가지를 설명한다.



### 6.5.1 LSTM 계층 다층화

정확한 모델을 만들고자 하면 대부분의 경우 LSTM 계층을 깊이 쌓아 효과를 볼 수 있다.![image-20210409150204142](CHAPTER6_GateRNN.assets/image-20210409150204142.png)

첫 번째 LSTM 계층의 은닉 상태가 두 번째 LSTM 계층에 입력된다. 계층을 더 쌓으면 더 복잡한 패턴을 학습할 수 있게 된다. 쌓는 층 수는 하이퍼파라미터이므로 처리할 문제의 복잡도나 준비된 학습 데이터의 양에 따라 적절하게 결정해야 한다.



### 6.5.2 드롭아웃에 의한 과적합 억제

층이 깊어지면 과적합 문제가 생길 수 있다. RNN은 일반적인 피드포워드 신경망보다 쉽게 과적합을 일으키므로 RNN의 과적합 대책은 중요하다.
...
![image-20210409150519994](CHAPTER6_GateRNN.assets/image-20210409150519994.png)

드롭아웃은 무작위로 뉴런을 선택하여 선택한 뉴런에서 그 앞 계층으로부터의 신호 전달을 막는다. 이 "무작위한 무시"가 제약이 되어 신경망의 일반화 성능을 개선한다.

RNN에서 드롭아웃 계층을 시계열 방향으로 넣어버리면 학습할 때 시간이 흐름에 따라 정보가 사라질 수 있다. 즉, 흐르는 시간에 비례해 드롭아웃에 의한 노이즈가 축적된다. 

그래서 드롭아웃 계층을 깊이 방향(상하 방향)으로 삽입하는 방안을 고려해보자.

![image-20210409150958252](CHAPTER6_GateRNN.assets/image-20210409150958252.png)

이렇게 구성하면 시간 방향(좌우 방향)으로 아무리 진행해도 정보를 잃지 않는다. 드롭아웃이 시간축과는 독립적으로 깊이 방향(상하 방향)에만 영향을 주는 것이다.



'일반적인 드롭아웃'은 시간 방향에는 적합하지 않다. 그러나 최근 연구에서는 RNN의 시간 방향 정규화를 목표로 하는 방법이 다양하게 제안되고 있다. (변형 드롭아웃)
...
![image-20210409162044367](CHAPTER6_GateRNN.assets/image-20210409162044367.png)

마스크란 데이터의 통과/차단을 결정하는 이진 형태의 무작위 패턴을 가리킨다.

같은 계층의 드롭아웃끼리 마스크를 공유함으로써 마스크가 '고정'된다. 그 결과 정보를 잃게 되는 방법도 '고정'되므로, 일반적인 드롭아웃 때와 달리 정보가 지수적으로 손실되는 사태를 피할 수 잇다.



### 6.5.3 가중치 공유

가중치 공유(weight tying)가 있다. Embedding 계층과 Softmax 앞단의 Affine 계층이 가중치를 공유한다.

![image-20210409162425213](CHAPTER6_GateRNN.assets/image-20210409162425213.png)

두 계층이 가중치를 공유함으로써 학습하는 매개변수 수가 크게 줄어드는 동시에 정확도도 향상되는 일석이조의 기술이다.

가중치 공유를 구현 관점에서 생각하면 어휘 수를 V로, LSTM의 은닉 상태의 차원 수를 H라고 해보자. Embedding 계층의 가중치는 형상이 VxH이며, Affine 계층의 가중치 형상은 HxV가 된다. 이 때 가중치 공유를 적용하려면 Embedding 계층의 가중치를 전치하여 Affine 게층의 가중치로 설정하기만 하면 된다.



### 6.5.4 개선된 RNNLM 구현

![image-20210409162716775](CHAPTER6_GateRNN.assets/image-20210409162716775.png)
...
한가지를 더 추가해보자면 매 에폭에서 검증 데이터로 퍼플렉서티를 평가하고, 그 값이 나빠졌을 경우에만 학습률을 낮추는 것이다. 이 기술은 실전에서 자주 쓰이며, 더 좋은결과로 이어지는 경우가 많았다.

매 에폭마다 검증 데이터로 퍼플렉서티를 평가하고, 그 값이 기존 퍼플렉서티(best_ppl)보다 낮으면 학습률을 1/4로 줄인다. 

LSTM의 다층화로 표현력을 높이고, 드롭아웃으로 범용성을 향상시켰으며, 가중치 공유로 가중치를 효율적으로 이용함으로써 큰 폭의 정확도 향상을 달성할 수 있었다.



### 6.5.5 첨단 연구로

첨단 모델에서도 다층 LSTM을 사용하고, 드롭아웃 기반의 정규화(변형 드롭아웃과 DropConnect) 를 수행하고, 가중치 공유도 수행한다. 또한 한층 더 세련된 최적화나 정규화 기법들이 적용되고, 또 하이퍼파라미터 튜닝도 매우 정밀하게 하고 있다.
...
- 게이트에는 전용 가중치가 있으며, 시그모이드 함수를 사용하여 0~1 사이의 실수를 출력한다.
