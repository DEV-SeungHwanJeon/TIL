...
CSS는 스타일, 레이아웃 등을 통해 문서(HTML)를 표시하는 방법을 지정하는 언어이다.
...
기본적으로 CSS에서 모든 요소의 box sizing 설정값은 content-box이다. content-box 설정에서는 테두리의 영역을 제외한 순수 content 영역만을 box로 지정한다. 하지만 우리는 테두리의 영역까지를 고려하고 싶다. 그래서 box-sizing이라는 속성에 border-box라고 설정하여 테두리 영역까지를 기준으로 잡는다.
